...
extension FolioReaderWebView {
  func stringByEvaluatingJavaScript(from: String) async -> String? {
    return await withCheckedContinuation { continuation in
      evaluateJavaScript(from) { response, error in
        ...
        guard let response = response as? String else {
          print("failed to cast response as string")
          continuation.resume(with: .success(nil))
          return
        }
...
open class FolioReaderPage: UICollectionViewCell, WKNavigationDelegate, UIGestureRecognizerDelegate {
  ...
  public func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
    ...
    // MARK: - UIWebView Delegate Revised

    open func webViewDidFinishLoad(_ webView: FolioReaderWebView) {
      Task {
        delegate?.pageWillLoad?(self)

        // Add the custom class based onClick listener
        await self.setupClassBasedOnClickListeners()

                refreshPageMode()

        if self.readerConfig.enableTTS && !self.book.hasAudio {
          await try? webView.evaluateJavaScript("wrappingSentencesWithinPTags()")
...
//  open func webView(_ webView: UIWebView, shouldStartLoadWith request: URLRequest, navigationType: UIWebView.NavigationType) async -> Bool {
//        guard
...
//                      await handleAnchor(anchorFromURL!, avoidBeginningAnchors: false, animated: true)
//                        return false
//                    }
//                } else {
...
//    }
    ...
    /**
     ...
     - parameter animated:              Enable or not scrolling animation
     */
    open func handleAnchor(_ anchor: String,  avoidBeginningAnchors: Bool, animated: Bool) async {
        if !anchor.isEmpty {
          let offset = await getAnchorOffset(anchor)
     ...
     */
    func getAnchorOffset(_ anchor: String) async -> CGFloat {
        let horizontal = self.readerConfig.scrollDirection == .horizontal
        let script = "getAnchorOffset('\(anchor)', \(horizontal.description))"

      do {
        if let strOffset = try await webView?.evaluateJavaScript(script) as? NSString {
          return CGFloat((strOffset).floatValue)
        }
    ...
    func audioMarkID(_ identifier: String) {
      ...
      currentPage.webView?.evaluateJavaScript("audioMarkID('\(playbackActiveClass)','\(identifier)')")
    }
...
//            }
    ...
    @objc func refreshPageMode() {
      Task {
        ...
        if (self.folioReader.nightMode == true) {
          ...
          let contentHeight = await webView.stringByEvaluatingJavaScript(from: script)
          let frameHeight = webView.frame.height
          let lastPageHeight = frameHeight * 2.0 /*CGFloat(webView.pageCount)*/ - CGFloat(Double(contentHeight!)!)
          colorView.frame = CGRect(x: webView.frame.width * 1/*CGFloat(webView.pageCount-1)*/, y: webView.frame.height - lastPageHeight, width: webView.frame.width, height: lastPageHeight)
    ...
    fileprivate func setupClassBasedOnClickListeners() async {
        for listener in self.readerConfig.classBasedOnClickListeners {
          await try? self.webView?.evaluateJavaScript("addClassBasedOnClickListener(\"\(listener.schemeName)\", \"\(listener.querySelector)\", \"\(listener.attributeName)\", \"\(listener.selectAll)\")")
        }
    }
     ...
     */
  open func performJavaScript(_ javaScriptCode: String) async -> String? {
        return await webView?.js(javaScriptCode)
    }
}
